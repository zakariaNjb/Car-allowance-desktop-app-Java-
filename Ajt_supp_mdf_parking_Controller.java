...
public class Ajt_supp_mdf_parking_Controller {
    ...
    @FXML javafx.scene.control.Button aff_list_parking;
    @FXML javafx.scene.control.Label comfir_label;/************comfirmation label**************************/

    @FXML javafx.scene.control.TextField n_parking;
    @FXML javafx.scene.control.TextField rue;
    @FXML javafx.scene.control.TextField arondiss;
    @FXML javafx.scene.control.TextField capacity;
    @FXML javafx.scene.control.TextField n_emptyplc;
    ...
    //function for "ajouter parking" :ajouter (Button)
    public void  ajt_parking(ActionEvent event){
        String n_park=n_parking.getText();
        String ru=rue.getText();
        String arrond=arondiss.getText();
        String cpct=capacity.getText();
        if(!n_park.isBlank() && !ru.isBlank() && !arrond.isBlank() && !cpct.isBlank()){
            try{
                int capt=Integer.parseInt(cpct);
                Parking parking=new Parking(n_park,ru,arrond,capt,capt);
                utilisateur utls=new utilisateur();
                boolean test=utls.ajouter_parking(parking);
                if(test) {
                    JOptionPane.showMessageDialog(null,"Opération est effectué!!");
                    n_parking.setText("");
                    rue.setText("");
                    arondiss.setText("");
                    capacity.setText("");
                ...
                JOptionPane.showMessageDialog(null,"Saisir la capacité de parking en format numérique!!");
            }
        }else if(n_park.isBlank()) JOptionPane.showMessageDialog(null,"Saisir le NR de parking!!");
        else if (ru.isBlank()) JOptionPane.showMessageDialog(null,"Saisir la rue où ce trouve le parking!!");
        else if(arrond.isBlank()) JOptionPane.showMessageDialog(null,"Saisir l'arrondissement où ce trouve le parking!!");
        else if(cpct.isBlank()) JOptionPane.showMessageDialog(null,"Saisir la capicité de parking!!");
    }
    //function:"chercher parking " chercher(Button)
    public void chr_parking(ActionEvent event){
        String n_park=n_parking.getText();
        if(!n_park.isBlank()){
            utilisateur utls=new utilisateur();
            Parking parking=utls.chercher_parking(n_park);
            if(parking!=null){
                n_parking.setText("NR_parking: "+parking.getN_parking());
                rue.setText("Rue: "+parking.getRue());
                arondiss.setText("Arrondissement: "+parking.getArrondissement());
                capacity.setText("Capacité: "+String.valueOf(parking.getCapacity()));
                n_emptyplc.setText("placV: "+ String.valueOf(parking.getNbr_place_vides()));
            }else JOptionPane.showMessageDialog(null,"le parking que vous cherchez n'est exist pas!!");
        }else JOptionPane.showMessageDialog(null,"Saisir le numéro de parking que vous cherchez!!");
    }
    //function:"supprimer parking" supprimer(Button)
    public void  sup_parking(ActionEvent event){
        String n_park=n_parking.getText();
        if(!n_park.isBlank()){
            utilisateur utls=new utilisateur();
            Parking parking=utls.chercher_parking(n_park);
            if(parking!=null){
                if (parking.getCapacity()-parking.getNbr_place_vides()==0){
                    boolean test=utls.supprimer_parking(n_park);
                    if(test){
                ...
                }else JOptionPane.showMessageDialog(null,"Opération n'est pas effectué ce parking contient des véhicule essayer de le vider avant la suppression!!");
            }else JOptionPane.showMessageDialog(null,"le parking que vous voulez supprimer n'est exist pas!!");
        }else JOptionPane.showMessageDialog(null,"Saisir le numéro de parking que vous voulez supprimer!!");
    }
    //function:"modifier parking" modiffier()
    public void mdf_parking(ActionEvent event){
        String n_park=n_parking.getText();
        if(!n_park.isBlank()){
            utilisateur utls=new utilisateur();
            Parking parking=utls.chercher_parking(n_park);
            if(parking!=null){
                if (rue.getText().isBlank() && arondiss.getText().isBlank() && capacity.getText().isBlank()) JOptionPane.showMessageDialog(null,"Saisi une autre information que vous voulez modifier!!");
                else {
                    if (!rue.getText().isBlank()) parking.setRue(rue.getText());
                    if (!arondiss.getText().isBlank()) parking.setArrondissement(arondiss.getText());
                    try {
                        if (!capacity.getText().isBlank()) {
                            int capct=Integer.parseInt(capacity.getText());
                            if (capct<parking.getCapacity()-parking.getNbr_place_vides()) JOptionPane.showMessageDialog(null,"Ce parking contient des véhicule:la capacité de ce parking doit etre superieur à le nombre de véhicule de ce parking!!");
                            else {
                                if (parking.getNbr_place_vides()==parking.getCapacity()) parking.setNbr_place_vides(capct);
                                parking.setCapacity(capct);
                                boolean test=utls.modifier_parking(parking);
            ...
            }else JOptionPane.showMessageDialog(null,"le parking que vous voulez modifier n'est exist pas!!");
        }else JOptionPane.showMessageDialog(null,"Saisir le numéro de parking que vous voulez modifier!!");
    }
    public void afficher_list_parking(ActionEvent event){
        Parent root = null;
        try {
            root = FXMLLoader.load(getClass().getResource("afficher_list_parking.fxml"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        Stage primaryStage=new Stage();
